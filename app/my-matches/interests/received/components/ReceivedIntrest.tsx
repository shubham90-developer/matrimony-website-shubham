...
// Only status rendered on this page. "Accepted", "Sent", and "Visitor" were
// removed along with their dead code — this page shows Received only.
      ...
      {actions && (
        <div className="flex gap-2 border-t border-stone-100 p-2.5">
          <button
            ...
            className="flex-1 cursor-pointer rounded-lg bg-emerald-500 py-2 text-xs font-semibold text-white transition hover:bg-emerald-600"
          >
            {actions.primary}
          </button>
...
export default function ReceivedInterests() {
  ...
  const receivedProfiles: CardProfile[] = received
  ...
  return (
    <div className="space-y-8 border border-gray-200 p-3">
      <PageHeader
        ...
        subtitle={`${receivedProfiles.length} people are interested in you`}
        imageSrc={"/img/logo/2.png"}
        imageAlt={receivedProfiles[0]?.name ?? "Profile"}
      />
